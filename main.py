...
# Incluindo o novo ID solicitado
ALVOS_IDS = [716390085896962058, 854233015475109888, 874910942490677270]
...
async def enviar_alerta(message):
    # Verifica se a mensagem veio de um bot de Pokémon
    is_bot = message.author.bot
    
    # Estética do Embed
    cor = 0xE91E63 if is_bot else 0x00AE86 # Rosa para bots, verde para o alvo
    embed = discord.Embed(
        title="🕵️ Detecção de Atividade",
        description=f"**Canal:** {message.channel.mention}\n**Link:** [Ir para Mensagem]({message.jump_url})",
        color=cor,
    ...
    # Lógica para capturar informações e imagens de Embeds (Bots)
    if message.embeds:
        for emb in message.embeds:
            # Pega o texto do bot se houver
            if emb.title: embed.add_field(name="📌 Título", value=emb.title, inline=False)
            if emb.description: embed.add_field(name="📝 Descrição", value=emb.description, inline=False)
            
            # Pega a imagem (Prioridade para imagem grande, depois thumbnail)
            if emb.image.url: url_img = emb.image.url
            elif emb.thumbnail.url: url_img = emb.thumbnail.url

    # Se não for embed, mas tiver texto normal
    elif conteudo_texto:
        embed.add_field(name="💬 Mensagem", value=conteudo_texto, inline=False)

    # Verifica se há anexos diretos (fotos enviadas por usuários)
    if not url_img and message.attachments:
        url_img = message.attachments[0].url

    # Processamento da Imagem para ficar DENTRO do Embed
    img_data = None
    if url_img:
        img_data = await download_image(url_img)
        if img_data:
            embed.set_image(url="attachment://pokemon_alerta.png")

    # Função para disparar os envios sem duplicar o consumo do arquivo
    async def disparar_envio(target_id, texto_personalizado=""):
        try:
            file = discord.File(io.BytesIO(img_data), filename="pokemon_alerta.png") if img_data else None
            await dm_cache[target_id].send(content=texto_personalizado, embed=embed, file=file)
        except Exception as e:
            print(f"Erro ao enviar para {target_id}: {e}")

    # Envio para você
    await disparar_envio(VOCE_ID)
    
    # Envio para sua amiga com a mensagem especial
    msg_amiga = "💖 **Yori:** Você é a pessoa mais especial e angelical que ja vi, tsu."
    await disparar_envio(AMIGA_ID, texto_personalizado=msg_amiga)

# --- EVENTOS ---

@bot.event
async def on_ready():
    print(f"✅ Monitoramento Online: {bot.user}")
    for uid in [AMIGA_ID, VOCE_ID]:
        try:
            user = await bot.fetch_user(uid)
            dm_cache[uid] = await user.create_dm()
        except:
            print(f"⚠️ Erro ao abrir DM com {uid}")
    check_recent.start()
